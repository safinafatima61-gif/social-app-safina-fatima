...
export default function PostsDashboard() {
  ...
  function togglePublic(post) {
    updatePost(post.id, { isPublic: !post.isPublic }, currentUser.id);
  }

  function publish(post) {
    updatePost(post.id, { isDraft: false, isPublic: true }, currentUser.id);
  }

  function confirmDelete() {
    if (deleteTarget) {
      deletePost(deleteTarget, currentUser.id);
  ...
  return (
    <div>
      <div className="mb-5 flex items-center justify-between">
        <h1 className="text-xl font-bold text-slate-900 dark:text-slate-50">My Posts</h1>
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {post.isDraft && (
                    <Button size="sm" onClick={() => publish(post)}>
                      Publish
                    </Button>
                  )}
                  {!post.isDraft && (
