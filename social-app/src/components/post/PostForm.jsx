...
export default function PostForm({ defaultValues = {}, onSubmit, submitting }) {
  ...
  const charColor =
    charCount >= 480 ? 'text-red-500' : charCount >= 400 ? 'text-amber-500' : 'text-slate-400';
    ...
    <form className="space-y-5">
      ...
      <div>
        ...
        <input
          ...
          className="block w-full text-sm text-slate-600 file:mr-3 file:rounded-lg file:border-0 file:bg-brand-50 file:px-3 file:py-2 file:text-sm file:font-medium file:text-brand-700 dark:text-slate-300 dark:file:bg-brand-950 dark:file:text-brand-300"
        />
        {imagePreview && (
          <div className="relative mt-3 inline-block">
            ...
            <button
              ...
              className="absolute -right-2 -top-2 grid h-7 w-7 place-items-center rounded-full bg-slate-900 text-white shadow"
      ...
      <div className="flex gap-3 pt-2">
        <Button variant="secondary" isLoading={submitting === 'draft'} onClick={submitAs(true)}>
          Save as Draft
        </Button>
        <Button variant="primary" isLoading={submitting === 'publish'} onClick={submitAs(false)}>
          Publish
        </Button>
      </div>
    </form>
